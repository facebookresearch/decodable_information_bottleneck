...
experiment="Qminimality2DBetaNoadd"
...
train.clf_kwargs.max_epochs=5000
...
kwargs_multi="
run=0,1,2
model=cdibexact,cdibhigher,cdibdet,vib
model.Q_zy.n_hidden_layers=2,1
...
for run in "0" "1" "2" 
do
for data in "binmnist" "bincifar100"
do
for beta in "0.1" "0.3" "1" "3" "10" "30"
do
for model in "cdibexact"   "cdibhigher" "cdibdet" "vib" 
do
  ...
  python load_models.py $kwargs \
        load_models.recolt_data.encoders_param=model.Q_zy.n_hidden_layers \
        load_models.recolt_data.encoders_vals=[2,1] \
...
python aggregate.py \
       ...
       plot_histories.folder_col=data \
